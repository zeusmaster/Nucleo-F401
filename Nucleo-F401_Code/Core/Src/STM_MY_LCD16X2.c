...
//***** Variables *****//
static const uint32_t writeTimeConstant = 10; // ORG VALUE =10
...
{
char numStr[16];
	int DigUnderzero; // 소수점 아래 숫자표시를 위한 변수
	switch(decimalPoints){
	case 1:
		DigUnderzero=(int)(number*10)%10;
		break;
	case 2:
		DigUnderzero=(int)(number*100)%100;
		break;
	case 3:
		DigUnderzero=(int)(number*1000)%1000;
		break;
	default:
		LCD1602_print("Only 0.1~0.001");
		break;
	}
	sprintf(numStr,"Temp : %d.%d", (int)number,DigUnderzero);
	LCD1602_print(numStr);
}
